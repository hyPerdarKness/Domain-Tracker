...
$kayit = $db->prepare("update defter set domain=?,text=? where id=?"); $kayit->execute(array($_POST['domain'], $_POST['text'], $id));

echo '<div class="alert alert-success">Düzenleme kaydedildi...</div>'; echo '<meta http-equiv="refresh" content="2">'; } } ?>
<form method="post">	
<?php foreach($db->query("select * from defter where id='".$id."'") as $lll){ ?>	
		<div class="control-group">											
			<label class="control-label">Domain</label>
			<div class="controls">
			<select class="span6" name="domain"><option><?php echo $lll['domain']; ?></option>
<?php foreach($db->query("select * from domain") as $x){ echo '<option>'.$x['domain'].'</option>'; } ?>
		...
		<div class="control-group">											
			...
			<div class="controls">
			<textarea class="span6" name="text" rows="5"><?php echo $lll['text']; ?></textarea>
			<span class="help-block">html kodları kullanabilirsiniz</span>
			</div>				
		</div>			

		<div class="form-actions">
			<button type="submit" name="kaydet" class="btn btn-info"><i class="icon-save"></i> Kaydet</button> 
